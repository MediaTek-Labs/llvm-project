...
private:
  ...
  // private may only be used by NanoMipsPropertyTable, to create them with
  // transform templates and ins properties
...
private:
  ...
  NanoMipsTransformTemplate(NanoMipsTransformType type,
    ...
    // for(uint32_t i = 0; i < insCount; i++) totalSizeOfTransform +=
    // insns[i].getSize();
  }
...
private:
  ...
  // I think 17 is max num of transform templates, but most of ins props have
  // less or equal to 8 NanoMipsTransformType is the key, but SmallDenseMap has
  // implementation for uint32_t keys
  llvm::SmallDenseMap<uint32_t, const NanoMipsTransformTemplate *, 8>
      transformationMap;
  // std::map<NanoMipsTransformType, const NanoMipsTransformTemplate *>
  // transformationMap;
...
public:
  ...
  const NanoMipsInsProperty *getInsProperty(uint64_t insn, uint64_t insnMask,
                                            RelType reloc,
  ...
  void updateSectionContent(InputSection *isec, uint64_t location,
                            int32_t delta, bool align = false) const {
    this->currentState->updateSectionContent(isec, location, delta, align);
  }
  void transform(Relocation *reloc,
                 const NanoMipsTransformTemplate *transformTemplate,
                 const NanoMipsInsProperty *insProperty,
    ...
    this->currentState->transform(reloc, transformTemplate, insProperty,
