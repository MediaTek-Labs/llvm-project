...
void elf::addReservedSymbols(Ctx &ctx) {
  if (ctx.arg.emachine == EM_MIPS || ctx.arg.emachine == EM_NANOMIPS) {
...
template <class ELFT> void Writer<ELFT>::setReservedSymbolSections() {
  ...
  if (ctx.sym.nanoMipsGp && !ctx.sym.nanoMipsGp->scriptDefined) {
    ...
    for (auto [i, osec] : llvm::enumerate(ctx.outputSections)) {
    ...
    if (foundSec == 5) {
      ctx.sym.nanoMipsGp->section = nullptr;
      ctx.sym.nanoMipsGp->value = 0;
    } else {
      ctx.sym.nanoMipsGp->section = ctx.outputSections[foundSecIdx];
      ctx.sym.nanoMipsGp->value = 0;
    }
  }
}
