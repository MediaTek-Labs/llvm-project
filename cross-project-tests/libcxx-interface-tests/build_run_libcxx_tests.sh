...
build_run_gettimeofday_dependent_test() {
        ...
        $toolchain/bin/nanomips-elf-g++ -c $SRC_DIR/$test_name.cpp -o $test_name.o -march=i7200 -Tuhi32.ld -fno-exceptions -lm -Wl,--defsym,__memory_size=256M
        if [ $? -ne 0 ]; then
        ...
        $toolchain/bin/nanomips-elf-gcc -c $SRC_DIR/../../libcxx/libstubs.c -o libstubs.o -march=i7200 -Tuhi32.ld -fno-exceptions -lm  -Wl,--defsym,__memory_size=256M
        if [ $? -ne 0 ]; then
        ...
        $toolchain/bin/nanomips-elf-g++ $test_name.o libstubs.o -o $test_name.elf -march=i7200 -Tuhi32.ld -fno-exceptions -lm  -Wl,--defsym,__memory_size=256M
        if [ $? -ne 0 ]; then
                echo -e ">>>>>>>>>>>>>>>>>>>> FAILED: $test_name <<<<<<<<<<<<<<<<<<<<\n"
                return
        fi
...
for test in ${tests[@]}; do 
	...
	if [[ $test == "chrono" || $test == "time" ]]; then
		...
		# Compile test
		$toolchain/bin/nanomips-elf-g++ $SRC_DIR/$test.cpp -o $test.elf -march=i7200 -Tuhi32.ld -fno-exceptions -lm -Wl,--defsym,__memory_size=256M
