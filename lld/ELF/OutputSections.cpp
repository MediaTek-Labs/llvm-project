  ...
  if (LLVM_UNLIKELY(type != isec->type)) {
    if (hasInputSections || typeIsSet) {
      if (typeIsSet || !canMergeToProgbits(type) ||
        ...
        // FIXME: A .rel.dyn section may be mapped to a SHT_PROGBITS section in a nanoMIPS linker script, so this is a workaround to let it pass
        auto diagnose = ((type == SHT_NOBITS) || (isec->name == ".rel.dyn" && config->emachine == EM_NANOMIPS)) ? warn : errorOrWarn;
