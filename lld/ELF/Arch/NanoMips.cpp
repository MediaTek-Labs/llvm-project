...
enum NanoMipsMach : uint32_t {
  MachNanoMipsIsa32R6 = 32,
  MachNanoMipsIsa64R6 = 64
};
...
inline bool doesNanoMipsMachExtend(uint32_t base, uint32_t extension) {
  ...
  if ((base == NanoMipsMach::MachNanoMipsIsa32R6) &&
      doesNanoMipsMachExtend(NanoMipsMach::MachNanoMipsIsa64R6, extension))
    return true;
...
uint32_t getNanoMipsMach(uint32_t eflags) {
  ...
  case E_NANOMIPS_ARCH_32R6:
    return NanoMipsMach::MachNanoMipsIsa32R6;
  case E_NANOMIPS_ARCH_64R6:
    return NanoMipsMach::MachNanoMipsIsa64R6;
  }
}
...
void NanoMips<ELFT>::relocateAlloc(InputSectionBase &sec, uint8_t *buf) const {
  ...
  for (auto it = sec.relocs().begin(), end = sec.relocs().end(); it != end;
    ...
    // If prev reloc was part of composite relocations
    // use its val for this reloc's addend
    // TODO: See if maybe addend should be
    // added to valFromBefore
    else
    ...
    // Note: linker transformations do not ruin this, as they don't contain
    // composite relocs, nor change them
    auto next = it + 1;
    if (next != sec.relocs().end() && next->offset == rel.offset &&
        !isPlaceholderReloc(next->type)) {
