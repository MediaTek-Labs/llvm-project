...
; RUN: llc -mtriple=nanomips-unknown=elf -enable-misched=false -mattr=+pcrel --disable-machine-licm -verify-machineinstrs < %s | FileCheck %s
...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    sync
; CHECK-DAG:    li $a[[REG1:[0-9]]], 1
; CHECK-DAG:    lapc.b $a[[REG2:[0-9]]], a
; CHECK-NEXT:  .LBB17_1: # %entry
; CHECK-NEXT:    # =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    ll $a2, 0($a[[REG2]])
; CHECK-NEXT:    bnec $a2, $a[[REG1]], .LBB17_3
; CHECK-NEXT:  # %bb.2: # %entry
; CHECK-NEXT:    # in Loop: Header=BB17_1 Depth=1
; CHECK-NEXT:    move $a3, $zero
; CHECK-NEXT:    sc $a3, 0($a[[REG2]])
