...
template <endianness E> uint32_t nanoMipsReadShuffle32(Ctx &ctx, const uint8_t *loc) {
...
void nanoMipsWriteShuffle32(Ctx &ctx, uint8_t *loc, uint64_t val) {
...
public:
  ...
  NanoMipsTransform(Ctx &ctx, const NanoMipsInsPropertyTable *tbl)
      : ctx(ctx), insPropertyTable(tbl) {}
...
public:
  NanoMipsTransformExpand(Ctx &ctx, const NanoMipsInsPropertyTable *tbl)
      : NanoMipsTransform(ctx, tbl) {
  ...
  SmallVector<NewInsnToWrite, 3> getTransformInsns(Relocation *reloc, 
...
public:
  NanoMipsTransformRelax(Ctx &ctx, const NanoMipsInsPropertyTable *tbl)
      : NanoMipsTransform(ctx, tbl) {
    assert(insPropertyTable);
  }
...
public:
  ...
  NanoMipsTransformNone(Ctx &ctx, const NanoMipsInsPropertyTable *)
      : NanoMipsTransform(ctx, nullptr) {}
...
public:
  NanoMipsTransformController(Ctx &ctx)
      : ctx(ctx), transformRelax(ctx, &insPropertyTable), transformExpand(ctx, &insPropertyTable),
        transformNone(ctx, &insPropertyTable), currentState(&transformNone) {}
...
private:
  ...
  bool mayRelax() const {
    ...
    return (!ctx.arg.relocatable && (ctx.arg.relax || ctx.arg.expand));
  }
