...
if [ $# -lt 1 ]; then
	echo "Usage: ./build_run_libcxx_tests <path_to_elf_toolchain> <path_to_qemu>"
	exit 1
fi
...
$toolchain/bin/nanomips-elf-llvm-ar cru libstubs.a libstubs.o libcxx_stubs.o
...
for test in ${tests[@]}; do 
	...
	# Compile test
	$toolchain/bin/nanomips-elf-clang++ $SRC_DIR/$test.cpp -o $test.elf -march=i7200 -Tuhi32.ld -L. -Wl,--whole-archive -lstubs -Wl,--no-whole-archive -lm -Wl,--defsym,__memory_size=256M
	...
	# Run test
	$sim -cpu I7200 -m 256 -semihosting -nographic -kernel $test.elf &> $test.out
