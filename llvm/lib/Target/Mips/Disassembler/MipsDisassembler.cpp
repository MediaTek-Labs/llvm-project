...
public:
  MipsDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx, bool IsBigEndian)
      : MCDisassembler(STI, Ctx),
        ...
        IsNanoMips(STI.hasFeature(Mips::FeatureNanoMips)),
...
static DecodeStatus DecodeDSPRNMRegisterClass(MCInst &Inst, unsigned RegNo,
                                              uint64_t Address,
                                              const MCDisassembler *Decoder);
...
static DecodeStatus
DecodeACC64DSPNMRegisterClass(MCInst &Inst, unsigned RegNo, uint64_t Address,
                              const MCDisassembler *Decoder);
...
static DecodeStatus DecodeHI32DSPNMRegisterClass(MCInst &Inst, unsigned RegNo,
                                                 uint64_t Address,
                                                 const MCDisassembler *Decoder);
...
static DecodeStatus DecodeLO32DSPNMRegisterClass(MCInst &Inst, unsigned RegNo,
                                                 uint64_t Address,
                                                 const MCDisassembler *Decoder);
...
static DecodeStatus DecodeMemZeroNM(MCInst &Inst,
				    unsigned Insn,
				    uint64_t Address,
				    const MCDisassembler *Decoder);
...
DecodeStatus MipsDisassembler::getInstruction(MCInst &Instr, uint64_t &Size,
  ...
  if (IsNanoMips) {
    ...
    if (Result != MCDisassembler::Fail) {
      LLVM_DEBUG(dbgs() << "Trying NanoMips32 table (32-bit instructions):\n");
      // Calling the auto-generated decoder function.
    ...
    if (Result != MCDisassembler::Fail) {
      LLVM_DEBUG(dbgs() << "Trying NanoMips16 table (16-bit instructions):\n");
...
static DecodeStatus DecodeMemZeroNM(MCInst &Inst,
				...
				const MCDisassembler *Decoder) {
  unsigned Base;
  Base = fieldFromInstruction(Insn, 0, 5);
  Base = getReg(Decoder, Mips::GPRNM32RegClassID, Base);
  Inst.addOperand(MCOperand::createReg(Base));

  return MCDisassembler::Success;
}

template <unsigned RegClass = Mips::GPRNM32RegClassID>
static DecodeStatus DecodeMemNMRX(MCInst &Inst,
				  unsigned Insn,
				  uint64_t Address,
				  const MCDisassembler *Decoder) {
  ...
  Base = getReg(Decoder, RegClass, Base);
  Offset = getReg(Decoder, RegClass, Offset);
  Inst.addOperand(MCOperand::createReg(Base));
  Inst.addOperand(MCOperand::createReg(Offset));
...
DecodeACC64DSPNMRegisterClass(MCInst &Inst, unsigned RegNo, uint64_t Address,
  ...
  unsigned Reg = getReg(Decoder, Mips::ACC64DSPNMRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus
DecodeHI32DSPNMRegisterClass(MCInst &Inst, unsigned RegNo, uint64_t Address,
                             const MCDisassembler *Decoder) {
  if (RegNo >= 4)
    return MCDisassembler::Fail;

  unsigned Reg = getReg(Decoder, Mips::HI32DSPNMRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus
DecodeLO32DSPNMRegisterClass(MCInst &Inst, unsigned RegNo, uint64_t Address,
                             const MCDisassembler *Decoder) {
  if (RegNo >= 4)
    return MCDisassembler::Fail;

  unsigned Reg = getReg(Decoder, Mips::LO32DSPNMRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
