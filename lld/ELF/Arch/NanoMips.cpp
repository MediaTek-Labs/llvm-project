...
template <endianness E> void writeImmOf48bitIns(Ctx &ctx, uint8_t *loc, uint64_t val) {
  ...
  if constexpr (E == llvm::endianness::big)
...
void writeValue16(Ctx &ctx, uint8_t *loc, uint64_t val, uint8_t bitsSize, uint8_t shift) {
  uint32_t instr = read16(ctx, loc);
...
void writePcRel16(Ctx &ctx, uint8_t *loc, uint64_t val, uint8_t bitsSize) {
...
// R_NANOMIPS_LO12
void checkVal(Ctx &ctx, uint8_t *loc, int64_t v, int n, const Relocation &rel,
              const Symbol *sym, uint32_t shift, bool signedVal) {

  if ((v & ((1LL << shift) - 1)) != 0) {
    error(getErrorLoc(ctx, loc) + "\tvalue: \t" + llvm::utohexstr(v) +
...
void NanoMips<ELFT>::relocate(uint8_t *loc, const Relocation &rel,
  ...
  switch (rel.type) {
  case R_NANOMIPS_PC_I32:
    checkInt(ctx, loc, val - 4, 32, rel);
    writeImmOf48bitIns<ELFT::Endianness>(ctx, loc, val - 4);
    break;
  case R_NANOMIPS_PC32:
  case R_NANOMIPS_32:
  case R_NANOMIPS_I32:
  case R_NANOMIPS_GPREL_I32:
    ...
    writeImmOf48bitIns<ELFT::Endianness>(ctx, loc, val);
    break;
  case R_NANOMIPS_NEG:
  case R_NANOMIPS_ASHIFTR_1:
    if (ELFT::Is64Bits)
  ...
  case R_NANOMIPS_GPREL_HI20:
    checkInt(ctx, loc, val, 32, rel);
    writeValueHi20<ELFT::Endianness>(ctx, loc, val);
    break;
  case R_NANOMIPS_PC4_S1: {
    checkVal(ctx, loc, val - 2, 5, rel, rel.sym, 1, false);
    val = (val - 2) >> 1;
    writeValue16(ctx, loc, val, 4, 0);
    break;
  }
  case R_NANOMIPS_SIGNED_8: {
  ...
  case R_NANOMIPS_UNSIGNED_8:
    ...
    break;
  case R_NANOMIPS_GPREL7_S2:
    checkVal(ctx, loc, val, 9, rel, rel.sym, 2, false);
    writeValue16(ctx, loc, val >> 2, 7, 0);
    break;
  case R_NANOMIPS_PC10_S1:
    checkVal(ctx, loc, val - 2, 11, rel, rel.sym, 1, true);
    writePcRel16(ctx, loc, val - 2, 10);
    break;
  case R_NANOMIPS_PC7_S1:
    checkVal(ctx, loc, val - 2, 8, rel, rel.sym, 1, true);
    writePcRel16(ctx, loc, val - 2, 7);
    break;
  case R_NANOMIPS_PC25_S1: {
    checkVal(ctx, loc, val - 4, 26, rel, rel.sym, 1, true);
    writePcRel32<ELFT::Endianness>(ctx, loc, val - 4, 25);
    break;
  }
  case R_NANOMIPS_PC21_S1:
    checkVal(ctx, loc, val - 4, 22, rel, rel.sym, 1, true);
    writePcRel32<ELFT::Endianness>(ctx, loc, val - 4, 21);
    break;
  case R_NANOMIPS_PC14_S1:
    checkVal(ctx, loc, val - 4, 15, rel, rel.sym, 1, true);
    writePcRel32<ELFT::Endianness>(ctx, loc, val - 4, 14);
    break;
  case R_NANOMIPS_PC11_S1:
    checkVal(ctx, loc, val - 4, 12, rel, rel.sym, 1, true);
    writePcRel32<ELFT::Endianness>(ctx, loc, val - 4, 11);
    break;
  case R_NANOMIPS_LO12:
    writeValue32Shifted<ELFT::Endianness>(ctx, loc, val, 12, 0);
    break;
  case R_NANOMIPS_LO4_S2:
    checkVal(ctx, loc, val & 0xfff, 6, rel, rel.sym, 2, false);
    writeValue16(ctx, loc, val >> 2, 4, 0);
    break;
  case R_NANOMIPS_GPREL19_S2:
    checkVal(ctx, loc, val, 21, rel, rel.sym, 2, false);
    writeValue32Shifted<ELFT::Endianness>(ctx, loc, val, 19, 2);
    break;
  case R_NANOMIPS_GPREL18:
    checkUInt(ctx, loc, val, 18, rel);
    writeValue32Shifted<ELFT::Endianness>(ctx, loc, val, 18, 0);
    break;
  case R_NANOMIPS_GPREL17_S1:
    checkVal(ctx, loc, val, 18, rel, rel.sym, 1, false);
    writeValue32Shifted<ELFT::Endianness>(ctx, loc, val, 17, 1);
    break;
  case R_NANOMIPS_GPREL_LO12:
    writeValue32Shifted<ELFT::Endianness>(ctx, loc, val, 12, 0);
    break;
...
void elf::setNanoMipsTargetInfo(Ctx &ctx){
  switch (ctx.arg.ekind) {
    case ELF32LEKind: {
      ctx.target.reset(new NanoMips<ELF32LE>(ctx));
      return;
    }
    case ELF32BEKind: {
      ctx.target.reset(new NanoMips<ELF32BE>(ctx));
      return;
    }
    case ELF64LEKind: {
      ctx.target.reset(new NanoMips<ELF64LE>(ctx));
      return;
    }
    case ELF64BEKind: {
      ctx.target.reset(new NanoMips<ELF64BE>(ctx));
      return;
    }
    default:
...
template <class ELFT> uint32_t NanoMips<ELFT>::calcEFlags() const {
  ...
  if ((retFlags & EF_NANOMIPS_PIC) == 0 &&
      (ctx.arg.pie || ctx.arg.shared || ctx.sharedFiles.size() != 0) &&
      !ctx.arg.relocatable)
